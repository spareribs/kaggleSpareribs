...
# 3.2.2 regional_mobility
train = pd.get_dummies(train, columns=["regional_mobility"], prefix="regional_mobility")
# 3.2.3 student_feature
train['student_feature'].fillna(0, inplace=True)
train = pd.get_dummies(train, columns=["student_feature"], prefix="student_feature")
# 3.2.4 is_high_user
train = pd.get_dummies(train, columns=["is_high_user"], prefix="is_high_user")
print(train.columns)

# 3.3 针对日期数据的处理(转换成年月日) - 后续处理，先直接删除 TODO
train = train.drop(["first_transaction_time", "latest_query_time", "loans_latest_time"], axis=1)

# 3.4 统计各个列标准差，将标准差小于0.1的特征剔除

print("{0}".format("*" * 20))
for feature in train.columns:  # 查看缺失的数据占比
    train[feature] = train[feature].astype('float64')  # 将所有数据转换成 float64类型的数据
    r = train[feature].std()
    print("{0:.2f}% {1}".format(r, feature))
    if r < 0.1:
...
for feature in train.columns:  # 查看缺失的数据占比
    ...
    print("{0:.2f}% {1}".format(null_count * 100 / len(train), feature))
